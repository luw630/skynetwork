...
function Game.init(helper, tabledata)
    ...
    local tablecreatelog = {
        ...
        create_name = table_conf.table_create_user_rolename,
        signup_cost = table_conf.signup_cost,
        promotion_blinds_time = table_conf.promotion_blinds_time,
        initial_chips = table_conf.initial_chips,
        player_num = table_conf.player_num,
        is_control = table_conf.is_control,
    }

    msghelper.write_table_mongolog("friendsngtablecreatelog", tablecreatelog)

    add_time_cost = configdao.get_businessconfitem_by_index(100, 1000, "friendsngcfg", "add_time_cost")
...
function Game.read_blindandawards_conf()
    ...
    blind_template = friendsngcfg.PromotionBlinds
    award_template = friendsngcfg.MatchRewards
end

function Game.set_current_blind()
    cur_blind_index = next_blind_index
    if cur_blind_index <= #blind_template then
        table_data.small_blinds = blind_template[cur_blind_index][1]
        table_data.big_blinds = blind_template[cur_blind_index][2]
        table_data.ante = 0
    end
end
...
function Game.get_nextupdateblind_time()
    ...
    local table_conf = msghelper.get_table_conf()
    return table_conf.promotion_blinds_time
end
...
function Game.gamestart()
    ...
    noticemsg.identify_code = table_conf.identify_code
    ...
    --拉入还没进入的玩家
    for rid, info in pairs(table_data.signup_list) do        
        if not Game.find_player_seat(rid) then
            local seat = msghelper.get_seat_byindex(0)
            seat.rid = rid
            seat.chips = table_conf.initial_chips
            seat.state = commonconst.PLAYER_STATE_WAIT_FOR_NEXT_ONE_GAME
            if table_data.wait_list[rid] then
                seat.gatesvr_id = table_data.wait_list[rid].gatesvr_id
                seat.agent_address = table_data.wait_list[rid].agent_address
            else
                seat.gatesvr_id = ""
                seat.agent_address = nil
                seat.is_tuoguan = true
            end
            seat.playerinfo.rolename = info.rolename
            seat.playerinfo.logo = info.logo
            seat.playerinfo.sex = info.sex
            seat.sng_rank = 0

            msghelper.sitdown(rid, seat.index, seat.chips, info.rolename)
            msghelper.del_wait_player(rid)

            --通知玩家开赛
            if seat.gatesvr_id == "" then
                msghelper.sendmsg_totableplayer_rid(rid, "friendsngstart", noticemsg)
            end
        end
    end
    ...
    signup_player_num = Game.get_signup_num()
    --清除sng桌子状态同步
    if sync_sngtablestate_timeid >= 0 then
        timer.cleartimer(sync_sngtablestate_timeid)
        sync_sngtablestate_timeid = -1
    end
    if table_data.cancelmatch_timer_id >= 0 then
        timer.cleartimer(table_data.cancelmatch_timer_id)
...
function Game.gameonerealend()
    ...
    for index, seat in pairs(table_data.tableseats) do
        if not Game.is_noplayer(seat) then
            if seat.chips <= 0 then
                ...
                table_data.signup_list[seat.rid] = nil                
                --将玩家站起
                Game.standup(seat)
            end
        end 
    ...
    if table_data.isdelete then
        ...
        if table_data.deletetable_timer_id < 0 then
            table_data.deletetable_timer_id = timer.settimer(60 * 100, "deletetable")
        end
        return
    end
...
function Game.gameend()
    ...
    Game.send_gametablerecord_notify()
    msghelper.save_tablesrecord()
...
function Game.grant_awardtoplayer(seat)
    ...
    if table_data.out_list[seat.rid] then
        return
    end

    local table_conf = msghelper.get_table_conf()
    local noticemsg = { rid = seat.rid, rank = seat.sng_rank}
    local award_list = award_template[signup_player_num] --有多少人参赛就发多少奖励
    local award_pool = signup_player_num * table_conf.signup_cost--奖池金额
    local award = 0

    if award_list and award_list[seat.sng_rank] then
        award = math.floor(award_list[seat.sng_rank] * award_pool)
        noticemsg.awards = {{id=1, num=award}}
    end

    Game.add_onetablerecord(seat, award)
    table_data.out_list[seat.rid] = true
    ...
    if seat.gatesvr_id ~= "" then
        ...
        if award==0 then
            --没有奖励
            local mailcfg = configdao.get_common_conf("friendsng_tuoguan_noaward_mail")

            local mail = {
                rid = seat.rid,
                create_time=timetool.get_time(),
                content=string.format(mailcfg, table_conf.table_create_user_rolename, seat.sng_rank),
            }
            playerdatadao.save_player_mail(seat.rid, mail)
        else
            --有奖励
            local mailcfg = configdao.get_common_conf("friendsng_tuoguan_award_mail")
            ...
            local mail = {
                rid = seat.rid,
                create_time=timetool.get_time(),
                content=string.format(mailcfg, table_conf.table_create_user_rolename, seat.sng_rank, json.encode(awards)),
            }
            playerdatadao.save_player_mail(seat.rid, mail)
        end
    end
...
function Game.analyse_and_senddoaction(index)
    ...
    if table_data.add_time_player_list[seat.rid] == nil then
        table_data.add_time_player_list[seat.rid] = 1
    end
    local next_addtime_index = table_data.add_time_player_list[seat.rid]
    if next_addtime_index > #add_time_cost then
        table_data.add_time_player_list[seat.rid] = #add_time_cost
        next_addtime_index = #add_time_cost
    end
    noticemsg.add_time_cost = add_time_cost[next_addtime_index]
...
--推送离线消息
function Game.pushofflinecache(rid)
    local table_conf = msghelper.get_table_conf()
    if table_conf.table_create_user == rid then
        --桌主上线了
        for _, record in pairs(table_data.applysignuprecords) do
            record.msg.timeout = record.msg.timeout - (timetool.get_time() - record.time)
            msghelper.sendmsg_totableplayer_rid(rid, "friendsngsignnotice", record.msg)
        end
...
--
function Game.pushsignupinfo(rid)
    local table_conf = msghelper.get_table_conf()
    local noticemsg = {
        is_signup = Game.is_signup(rid),
        is_signuping = Game.is_signuping(rid),
        signup_cost = table_conf.signup_cost,
        service_fee = table_conf.service_fee,
        promotion_blinds_time = table_conf.promotion_blinds_time,
        initial_chips = table_conf.initial_chips,
        small_blinds = table_data.small_blinds,
        big_blinds = table_data.big_blinds,
        signup_max_num = table_conf.player_num,
        signup_list = {},
        refuse_time = Game.get_refuse_time(rid)
    }

    if not Game.is_gameend() then
        noticemsg.signup_num = signup_player_num
    else
        noticemsg.signup_num = Game.get_signup_num()
    end

    for _, info in pairs(table_data.signup_list) do
        table.insert(noticemsg.signup_list, info.rolename)
    end
    msghelper.sendmsg_totableplayer_rid(rid, "friendsngsignupinfo", noticemsg)
end

--玩家主动操作
function Game.onentertable(rid)
    Game.pushsignupinfo(rid)
    Game.pushofflinecache(rid)
    msghelper.write_tableinfo_log("onentertable", table_data)

    local seat = msghelper.find_player_seat(rid)
    if seat then
        Game.onreentertable(rid, seat.index)
    end
end
...
function Game.onreentertable(rid, seat_index)
    ...
    local next_addtime_index = table_data.add_time_player_list[seat.rid]
    if next_addtime_index ~= nil 
        and next_addtime_index > #add_time_cost then
        table_data.add_time_player_list[seat.rid] = #add_time_cost
        next_addtime_index = #add_time_cost
        noticemsg.add_time_cost = add_time_cost[next_addtime_index]
    end
...
function Game.restore_signupfee(rid)
    local mail = configdao.get_common_conf("friendsng_signfees_mail")
    local table_conf = msghelper.get_table_conf()
    local wait_player = table_data.wait_list[rid]

    if not wait_player.signuping 
        or not wait_player.deduct_signup_cost then
        return
    end
...
function Game.onleavetable(rid)
    --如果游戏已经结束，当所有人都离开后，清除数据
    if table_data.isdelete and msghelper.is_noplayer() and tabletool.is_emptytable(table_data.wait_list) then
        if table_data.deletetable_timer_id >= 0 then
            --去掉之前的timer，立即结束
            timer.cleartimer(table_data.deletetable_timer_id)
        end
    ...
    if Game.is_signuping(rid) then
        local wait_player = table_data.wait_list[rid]
        local table_conf = msghelper.get_table_conf()
        local responsemsg = {
            rid = rid,
            rolename = wait_player.rolename,
            table_id = table_data.table_id,
        }

        Game.restore_signupfee(rid)
        
        msghelper.sendmsg_totableplayer_rid(table_conf.table_create_user, "cancelfriendsngsignnotice", responsemsg)
    end

    msghelper.write_tableinfo_log("onleavetable", table_data)
end

function Game.find_player_seat(rid)
...
function Game.kickfromtable(rid)
    local seat_index = Game.find_player_seat(rid)
    if seat_index then
        Game.onkickplayer(0, seat_index, 1)
        return true
    else
        if table_data.wait_list[rid] then
            Game.leavetable(rid, 1)
            return true
        end
        return false
    end
...
function Game.onkickplayer(fromrid, seat_index, reason)
    ...
    if Game.is_ingameseat(seat) then
        ...
        resutdata.expchange = 0
        resutdata.room_type = table_data.table_room_type
        resutdata.table_id = table_data.table_id
        resutdata.table_create_time = table_conf.table_create_time
        resutdata.bunko = 0
        resutdata.winchips = 0
        resutdata.rid = seat.rid
    ...
    Game.leavetable(noticemsg.rid, reason)
...
--将玩家主动从座位上站起
function Game.onstandup(seat_index, cmd, reason)
    local seat = msghelper.get_seat_byindex(seat_index)
    ...
    local seat_state
    if Game.is_gameend() then
        seat_state = commonconst.PLAYER_STATE_NO_PLAYER        
    else
        seat_state = commonconst.PLAYER_STATE_SNG_OUT
    end
    noticemsg.seat_state = seat_state
    noticemsg.sng_rank = seat.sng_rank
    msghelper.sendmsg_toalltableplayer("standup", noticemsg)
    seat.state = seat_state
    -- 加入等待列表
    -- 玩家没有状态的不能进入等待列表
    if table_data.wait_list[seat.rid] == nil then
        local wait_player = {
...
function Game.onstartgame()
    local count = Game.get_signup_num()
...
function Game.onupdateblind(timerid)
    ...
    if next_blind_index < #blind_template then
        local noticemsg = {}
        noticemsg.small_blinds = blind_template[next_blind_index+1][1]
        noticemsg.big_blinds = blind_template[next_blind_index+1][2]
        noticemsg.current_blind_index = cur_blind_index
        noticemsg.next_blind_index = next_blind_index + 1
        noticemsg.ante = 0
        if next_blind_index + 2 > #blind_template then
            ...
            local table_conf = msghelper.get_table_conf()
            noticemsg.next_updatetime = table_conf.promotion_blinds_time
        end
        noticemsg.is_first = false
        msghelper.sendmsg_toalltableplayer("matchblindupdate", noticemsg)
...
gametablerecord 96 {
    response {
        tablerecord 0 : TableRecordItem
        isclose 1 : boolean   #表示朋友桌是否结束 true 是，false 否
        timestamp 2 : integer #牌局结束时间 
    }
}
]]
function Game.send_gametablerecord_notify()
    local noticemsg = {isclose = false, timestamp = timetool.get_time(),}
    local table_record_item = {}    
    local table_conf = msghelper.get_table_conf()
    table_record_item.table_room_type = table_conf.table_room_type
    ...
    table_record_item.match_identify_code = table_conf.identify_code
    table_record_item.signup_fees = {{id=1, num=table_conf.signup_cost}}
    noticemsg.isclose = true
    noticemsg.tablerecord = table_record_item    
    msghelper.sendmsg_toalltableplayer("gametablerecord", noticemsg)

    commondatadao.save_friendtable_record(tostring(table_record_item.create_user_rid)..tostring(table_record_item.create_time), table_record_item)
        
    msghelper.write_table_mongolog("tablesrecord", table_record_item)
end
...
--添加玩家战绩
function Game.add_onetablerecord(seat, reward)
    local table_conf = msghelper.get_table_conf()
    ...
    record.rank = seat.sng_rank
    record.reward = reward
    record.buy_in_chips = seat.carry_chips
    record.signup_fees = {{id=1, num = table_conf.signup_cost}}
    if seat.rid ~= 0 then
        table_data.tablesrecord[seat.rid] = record
    end
end
...
function Game.is_gameend()
    ...
    if tablestate == commonconst.TABLE_STATE_WAIT_GAME_START or tablestate == commonconst.TABLE_STATE_GAME_END then
        return true
    end
    return false
end
...
function Game.get_signup_num()
    return tabletool.getn(table_data.signup_list)
end

function Game.is_signup(rid)
    return table_data.signup_list[rid]~=nil
end

function Game.is_signuping(rid)
    local wait_player = msghelper.find_wait_player(rid)
    if wait_player then
        return wait_player.signuping
    end
end

function Game.get_refuse_time(rid)
    local refuse_time = 0
    local refuseinfo = table_data.refusesignuprecords[rid]
    if refuseinfo then
        local refuse_time_cfg = configdao.get_common_conf("friendsng_refuse_signup_time")
        refuse_time = refuseinfo.time + refuse_time_cfg * 60 - timetool.get_time()
    end

    return refuse_time
end

function Game.onsignup(rid)
    local table_conf = msghelper.get_table_conf()
    if table_data.signup_list[rid] == nil 
        and table_data.wait_list[rid] ~= nil then
        local deduct_signup_cost = table_conf.signup_cost + table_conf.service_fee
        table_data.signup_list[rid] = {
            rolename = table_data.wait_list[rid].rolename,
            logo = table_data.wait_list[rid].logo,
            sex = table_data.wait_list[rid].sex,
            deduct_signup_cost = deduct_signup_cost,
        }
    end
    ...
    --广播报名信息
    msghelper.sendmsg_toallplayer("friendsngsignupinfo", function(rid)
...
function Game.onfriendsngsign(rid)
    ...
    local deduct_signup_cost = table_conf.signup_cost + table_conf.service_fee
    if table_conf.is_control 
        and rid ~= table_conf.table_create_user then
        --需要控制，发送给桌主
        local refuseinfo = table_data.refusesignuprecords[rid]
        if refuseinfo then
            --判断次数
            local refuse_count = configdao.get_common_conf("friendsng_refuse_signup_count")
            if refuseinfo.count >= refuse_count then
                responsemsg.issucces = false
                responsemsg.resultdes = string.format("拒绝%s次后不能申请报名！", refuse_count)
                return responsemsg
            end
            --判断时间
            local refuse_time = configdao.get_common_conf("friendsng_refuse_signup_time")
            if timetool.get_time() < refuseinfo.time + refuse_time*60 then
                responsemsg.issucces = false
                responsemsg.resultdes = string.format("拒绝%d分钟内不能申请报名！", refuse_time)
                return responsemsg
            end
        end

        local timeout = configdao.get_common_conf("friendsng_apply_signup_timeout")
        ...
        noticemsg = {
            ...
            table_id = table_data.table_id,
            identify_code = table_conf.identify_code,
            roomsvr_id = msghelper.get_roomsvr(),
            table_address = skynet.self(),
        }

        if not msghelper.sendmsg_totableplayer_rid(table_conf.table_create_user, "friendsngsignnotice", noticemsg) then
            -- local record = {
            --     msg = noticemsg,
            --     time = timetool.get_time(),
            -- }
            -- table_data.applysignuprecords[rid] = record
            --桌主不在线返回失败
            responsemsg.issucces = false
            responsemsg.resultdes = "您在当前朋友桌的申请操作由于桌主不在线导致申请失败"
            return responsemsg
        end

        wait_player.signuping = true
        wait_player.deduct_signup_cost = deduct_signup_cost
        if wait_player.signup_timerid ~= nil 
            and wait_player.signup_timerid >= 0 then
            timer.cleartimer(wait_player.signup_timerid)
        end
        wait_player.signup_timerid = timer.settimer(timeout*100, "friendsngsign", rid)

        responsemsg.need_agree = true
        responsemsg.timeout = timeout
        responsemsg.table_id = table_data.table_id
        responsemsg.chips = table_conf.signup_cost + table_conf.service_fee
        return responsemsg
    end

    --不需要控制
    responsemsg.need_agree = false

    table_data.signup_list[rid] = table_data.wait_list[rid]
    ...
    msghelper.sendmsg_totableplayer_rid(rid, "friendsngsignresult", notifymsg)

    --扣除筹码
    responsemsg.chips = deduct_signup_cost
    table_data.signup_list[rid].deduct_signup_cost = deduct_signup_cost
    return responsemsg
end

function Game.getfriendsngrules()
    local responsemsg = {issucces = true, rank_award = {}, blinds_list = {}}

    local table_conf = msghelper.get_table_conf()

    --实际参与人数
    local player_num = Game.get_signup_num()
    --奖励列表
    local award_list = award_template[player_num]
    if award_list then
        --总奖池
        local award_pool = player_num * table_conf.signup_cost
        for rank, ratio in ipairs(award_list) do
            local award = math.floor(ratio * award_pool)
            table.insert(responsemsg.rank_award, {rank=rank, award=award})
        end
    end

    --升盲表
    for _, blinds in ipairs(blind_template) do
        table.insert(responsemsg.blinds_list, {small_blinds=blinds[1], big_blinds=blinds[2], updatetime=table_conf.promotion_blinds_time})
    end
...
function Game.cancelmatch()
    local table_conf = msghelper.get_table_conf()
    local mailcfg = configdao.get_common_conf("friendsng_signfees_mail")
    local signup_cost = {{id=1,}}
    local mail
    for rid, info in pairs(table_data.signup_list) do
        --发退费邮件
        info.deduct_signup_cost = info.deduct_signup_cost or 0
        signup_cost[1].num=info.deduct_signup_cost
        mail = {
            rid = rid,
            create_time=timetool.get_time(),
            content=string.format(mailcfg,
                table_conf.table_create_user_rolename,
                info.deduct_signup_cost,
                json.encode(signup_cost)),
        }
        playerdatadao.save_player_mail(rid, mail)
    end

    for rid, info in pairs(table_data.wait_list) do
        --发退费邮件
        if info.deduct_signup_cost then
            signup_cost[1].num=info.deduct_signup_cost
            mail = {
                rid = rid,
                create_time=timetool.get_time(),
...
function Game.get_add_time_cost()
    return add_time_cost
end

function Game.add_time(seat, add_time)
    -- 设置table状态
    if table_data.timer_id >= 0 then
        timer.cleartimer(table_data.timer_id)
        table_data.timer_id = -1
    end
    table_data.action_timeout = table_data.action_timeout + add_time
    table_data.action_index = seat.index

    --设置玩家操作定时器
    local doaction_timermsg = {}
    doaction_timermsg.rid = seat.rid
    doaction_timermsg.seat_index = seat.index
    local timerid = timer.settimer((table_data.action_timeout - timetool.get_time())*100, "doaction", doaction_timermsg)
    if table_data.table_state ~= commonconst.TABLE_STATE_WAIT_CLIENT_ACTION and table_data.action_index ~= seat.index then
        timer.cleartimer(timerid)
        return false
    end 
    table_data.timer_id = timerid
