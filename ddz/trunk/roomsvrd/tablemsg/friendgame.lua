...
function Game.init(helper, tabledata)
    ...
    local tablecreatelog = {
        ...
        big_blinds = table_conf.big_blinds,
        retain_time = table_conf.retain_time,
        is_control = table_conf.is_control,
        action_timeout = table_conf.action_timeout,
    }
    msghelper.write_table_mongolog("tablecreatelog", tablecreatelog)

    add_time_cost = configdao.get_businessconfitem_by_index(100, 1000, "friendgamecfg", "add_time_cost")
...
function Game.gameonestart()
    ...
    for _, seat in ipairs(table_data.tableseats) do
        if Game.is_ingameseat(seat) then
            local autorebuychips = table_data.autobuyinplayers[seat.rid]
            if autorebuychips then
                seat.chips = autorebuychips + seat.chips
                table_data.autobuyinplayers[seat.rid] = nil

                --写入数据
                seat.outtable_chips = seat.outtable_chips - autorebuychips
                msghelper.save_user_rebuychips(seat.rid, -autorebuychips, seat.chips)

                --写入动态
                msghelper.add_tablestateinfo(ETableStateinfoType.TABLE_STATEINFO_REBUY, seat.rid, autorebuychips, timetool.get_time(), seat.playerinfo.rolename)

                --写入战绩
                local record = {}
                record.rid = seat.rid
                record.buy_in_chips = autorebuychips
                record.win_chips = 0
                record.rolename = seat.playerinfo.rolename
                record.logo = seat.playerinfo.logo
                Game.add_onetablerecord(seat.rid, record)
            end
...
function Game.gameoneend()
    ...
    for index, seat in ipairs(table_data.tableseats) do
        if Game.is_ingameseat(seat) then
            ...
            local record = {}
            record.rid = seat.rid
            record.buy_in_chips = 0
            record.win_chips = seat.onegame_winchips
            record.rolename = seat.playerinfo.rolename
            record.logo = seat.playerinfo.logo
            Game.add_onetablerecord(seat.rid, record)

            table_data.fixedchipsplayers[seat.rid]=seat.chips
        end
    end 
    ...
    for index, seat in pairs(table_data.tableseats) do
        if not Game.is_noplayer(seat) then
            ...
            if seat.chips <= gamedraw and (seat.outtable_chips - seat.chips < gamedraw) then
                if not table_data.isdelete then
                    Game.standup(seat)
                end
            --如果剩余筹码不够小盲和前注则提出
            elseif seat.chips <= 0 and autorebuychips == nil then
                if not table_data.isdelete then
                    Game.standup(seat)
                end
...
function Game.pushofflinecache(rid)
    ...
    if table_conf.table_create_user == rid then
        --桌主上线了
        for _, record in pairs(table_data.applyenterrecords) do
            record.msg.timeout = record.msg.timeout - (timetool.get_time() - record.time)
            msghelper.sendmsg_totableplayer_rid(rid, "applyenterfriendtablenotify", record.msg)
        end
...
--玩家被动离开
function Game.leavetable (rid, reason)
...
function Game.standup(seat)
    ...
    msghelper.add_tablestateinfo(ETableStateinfoType.TABLE_STATEINFO_STANDUP, seat.rid, seat.chips, timetool.get_time(), seat.playerinfo.rolename)
...
function Game.onstandup(seat_index, cmd, reason)
    ...
    if seat.state == commonconst.PLAYER_STATE_WAIT_CONFIRM then
        ...
        table_data.applyenterrecords[seat.rid] = nil

        if cmd~="dont_notify_cancel" then
            local table_conf = msghelper.get_table_conf()
            msghelper.sendmsg_totableplayer_rid(table_conf.table_create_user, "applyenterfriendtablecancel", responsemsg)
        end

        --解除筹码锁定
        msghelper.refund_user_fixedchips(seat.rid)
        table_data.fixedchipsplayers[seat.rid] = nil
    end
    ...
    noticemsg.seat_state = commonconst.PLAYER_STATE_NO_PLAYER
    if seat.chips > 0 and seat.state ~= commonconst.PLAYER_STATE_WAIT_CONFIRM then
        noticemsg.isfixedchips = true
    end
    msghelper.sendmsg_toalltableplayer("standup", noticemsg)
...
function Game.onsitdown(rid, seat_index)
    ...
    --如果carry_chips为0说明是之前有锁定筹码，可以直接开始
    --桌主也可以直接开始
    local table_conf = msghelper.get_table_conf()
    if table_conf.is_control and seat.carry_chips ~= 0 and rid ~= table_conf.table_create_user then
        --控制模式，需要桌主同意后才能开始游戏
        local timeout = configdao.get_common_conf("friendtable_apply_sitdown_timeout")

        local noticeemsg = {
            rid = rid,
            rolename = seat.playerinfo.rolename,
            carry_chips = seat.carry_chips,
            timeout = timeout,
            table_id = table_data.table_id,
            identify_code = table_conf.identify_code,
        ...
        if not msghelper.sendmsg_totableplayer_rid(table_conf.table_create_user, "applyenterfriendtablenotify", noticeemsg) then
            --如果桌主不在线，先保存起来
            -- local record = {
            --     msg = noticeemsg,
            --     time = timetool.get_time(),
            -- }
            -- table_data.applyenterrecords[rid] = record
        end
        --倒计时timeout
        seat.state = commonconst.PLAYER_STATE_WAIT_CONFIRM
        seat.wait_confirm = true


        msghelper.sendmsg_totableplayer(seat, "enterfriendtableneedcreatoragree", {timeout=timeout, table_id=table_data.table_id})

        seat.applyenter_timerid = timer.settimer(timeout*100, "applyenterfriendtable", rid)
    else
        seat.state = commonconst.PLAYER_STATE_WAIT_FOR_NEXT_ONE_GAME
    end
    ...
    table_data.fixedchipsplayers[seat.rid] = seat.carry_chips
...
function Game.game_continuous_timeout(seat_index)
    ...
    if seat.timeout_times ~= nil and seat.timeout_times >= table_data.continuous_timeout then
        --强制站起
        Game.onstandup(seat_index, nil, 1) --1代表因为挂机被踢出
...
function Game.is_ingameseat(seat)
    if seat.state ~= commonconst.PLAYER_STATE_NO_PLAYER and 
        seat.state ~= commonconst.PLAYER_STATE_WAIT_FOR_NEXT_ONE_GAME and
        seat.state ~= commonconst.PLAYER_STATE_WAIT_CONFIRM
    then
        return true
    end
...
function Game.is_gameend()
    ...
    if tablestate == commonconst.TABLE_STATE_WAIT_GAME_START then
        return true
    end
    return false
end
...
--添加玩家战绩
function Game.add_onetablerecord(rid, record)
    if table_data.tablesrecord[rid] == nil then
        table_data.tablesrecord[rid] = record
    else
        local tmprecord = table_data.tablesrecord[rid]
        tmprecord.buy_in_chips = tmprecord.buy_in_chips + record.buy_in_chips 
        tmprecord.win_chips = tmprecord.win_chips + record.win_chips
        tmprecord.rolename = record.rolename
        tmprecord.logo = record.logo
    end
end
...
--重购
function Game.onfriendtablerebuy(seat, chips)
    if (chips + seat.chips) > table_data.max_carry then
        return false, "筹码过大"
    end
    if chips > seat.outtable_chips then
        return false, "聚众币不足"
    end
    ...
    local table_conf = msghelper.get_table_conf()
    if table_conf.is_control and seat.rid ~= table_conf.table_create_user then
        --需要控制，发送给桌主

        local refuserebuy = table_data.refuserebuyrecords[seat.rid]
        if refuserebuy then
            --判断次数
            local refuse_rebuy_count = configdao.get_common_conf("friendtable_refuse_rebuy_count")
            if refuserebuy.count >= refuse_rebuy_count then
                return false, string.format("拒绝%s次后不能申请重构！", refuse_rebuy_count)
            end
            --判断时间
            local refuse_rebuy_time = configdao.get_common_conf("friendtable_refuse_rebuy_time")
            if timetool.get_time() < refuserebuy.time + refuse_rebuy_time*60 then
                return false, string.format("拒绝%d分钟内不能申请重构！", refuse_rebuy_time)
            end
        end

        local timeout = configdao.get_common_conf("friendtable_apply_rebuy_timeout")
        ...
        if not msghelper.sendmsg_totableplayer_rid(table_conf.table_create_user, "friendtablenoticerebuy", responsemsg) then
            return false, "您在当前朋友桌的申请操作由于桌主不在线导致申请失败"
        end

        seat.rebuying = chips

        msghelper.sendmsg_totableplayer(seat, "friendtablerebuyneedcreatoragree", {timeout=timeout, table_id=table_data.table_id})

        seat.rebuy_timerid = timer.settimer(timeout*100, "friendtablerebuy", seat.rid)
        return true
    end

    --不需要控制
    table_data.autobuyinplayers[seat.rid] = chips
    --回复玩家
    local notifymsg = {is_agree=true, ia_auto=true}
    msghelper.sendmsg_totableplayer(seat, "friendtablerebuyresult", notifymsg)

    --filelog.sys_info("Game.onfriendtablerebuy4", seat, chips)

    return true
end


function Game.checkgamerun()
    --判断游戏开始
    local count = Game.get_waitfornextonegameplayernum()
