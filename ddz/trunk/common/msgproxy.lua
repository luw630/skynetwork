...
--------------tablesvrd交互相关--------------------
function MsgProxy.send_reqmsgto_tablesvrd(rid, ...)
	init_svrs_indexs()
	local svrs = svrs_indexs["tablesvrs"]
...
function MsgProxy.send_broadcastmsgto_tablesvrd(...)
	init_svrs_indexs()
	local svrs = svrs_indexs["tablesvrs"]
...
------------------roomsvrd交互相关----------------------
function MsgProxy.send_reqmsgto_roomsvrd(rid, roomsvr_id, service_id, ...)	
	init_svrs_indexs()
	if service_id == nil then
		--以后根据rid做hash分配
		roomsvr_id = "roomsvr_1"
		service_id = "roomsvr_1"
	end
	if rid ~= nil then
	...
	if not status then
		--filelog.sys_error("MsgProxy.send_reqmsgto_roomsvrd", result1)
		return nil
	end
	return result1, result2, result3, result4
end

function MsgProxy.send_noticemsgto_roomsvrd(roomsvr_id, service_id, ...)
	init_svrs_indexs()
	if skynet.getenv("svr_id") ~= roomsvr_id 
		and cfgclusters[roomsvr_id] then
...
function MsgProxy.sendrpc_reqmsgto_roomsvrd(rid, roomsvr_id, service_id, ...)
	...
	if skynet.getenv("svr_id") ~= roomsvr_id 
		...
		status, result1, result2, result3, result4 = base.pcall(skynet.call, roomsvr_id, service_id, "request", ...)
	end
	if not status then
...
function MsgProxy.sendrpc_reqmsgto_datadbsvrd(rid, ...)
	init_svrs_indexs()
	local status, result1, result2, result3, result4
	local svrs = svrs_indexs["datadbsvrs"]
	local index = rid % (#svrs) + 1
	local svr_id = svrs[index]
	status, result1, result2, result3, result4 = base.pcall(skynet.call, proxy, "lua", "request", svr_id, svr_id, ...)		
	if not status then
