...
local filename = "playerstatesvrhelper.lua"
local PlayerStatesvrHelper = helperbase:new({})

function PlayerStatesvrHelper:get_roomsvr_states()
	msgproxy.sendrpc_broadcastmsgto_roomsvrd("get_roomsvr_states") 	
end 

function PlayerStatesvrHelper:clear_roomsvrd(roomsvr_id)
...
function PlayerStatesvrHelper:tick()
...
function PlayerStatesvrHelper:start_time_tick()
	skynet.fork(function()
		while true do
			skynet.sleep(1000)
			PlayerStatesvrHelper:tick()
		end
	end)	
end

return	PlayerStatesvrHelper 