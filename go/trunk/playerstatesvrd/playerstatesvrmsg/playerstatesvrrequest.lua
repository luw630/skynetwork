...
local PlayerStatesvrRequest = {}

function PlayerStatesvrRequest.process(session, source, event, ...)
	local f = PlayerStatesvrRequest[event] 
	if f == nil then
		return
	end
	f(...)
...
function PlayerStatesvrRequest.getPlayerstatebycreateid(request)
	...
	responsemsg.Playerstate = tableinfo
	base.skynet_retpack(responsemsg)
end

function PlayerStatesvrRequest.getfriendtablelist(request)
	local server = msghelper:get_server()	
	local table_pool = server.table_pool
	local tableinfo
